...
class EduMailBot {
    constructor(sessionDb) {
        ...
        // Memuat cookie dari MongoDB jika ada
            ...
            if (messagesArray && messagesArray[0] && messagesArray[0][0]) {
                ...
                for (let mail of mailItems) {
                    if (mail && mail.id && !readEmails.includes(mail.id)) {
                        let cleanContent = mail.content ? mail.content.replace(/<[^>]*>?/gm, '').trim() : "Kosong";
                        let otpMatch = cleanContent.match(/\b\d{6}\b/);
